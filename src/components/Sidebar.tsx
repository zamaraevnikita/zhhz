...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <>
            ...
            flex flex-col-reverse lg:flex-row lg:h-full bg-white/80 backdrop-blur-3xl shadow-[0_4px_30px_rgba(0,0,0,0.05)] lg:border-r border-white/60 transition-all duration-300 ease-in-out
            inset-x-0 bottom-0 lg:inset-auto lg:top-0 lg:left-0
            ${isPanelOpen ? 'h-[55vh] sm:h-[60vh] lg:w-[336px]' : 'h-14 lg:w-14'}
        `}>
                {/* ICON STRIP */}
                <div className="bg-transparent z-50 flex items-center lg:flex-col py-0 lg:py-6 gap-2 sm:gap-5 justify-around lg:justify-start px-4 lg:px-0 w-full lg:w-14 h-14 lg:h-full border-t border-black/5 lg:border-t-0 flex-shrink-0 relative">
                    <button onClick={() => handleTabClick('gallery')} className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${activeTab === 'gallery' && isPanelOpen ? 'bg-slate-900 text-white shadow-md shadow-slate-900/20' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-100 hover:scale-105 active:scale-95'}`}><Icons.Image size={18} strokeWidth={2} /></button>
                    <button onClick={() => handleTabClick('templates')} className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${activeTab === 'templates' && isPanelOpen ? 'bg-slate-900 text-white shadow-md shadow-slate-900/20' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-100 hover:scale-105 active:scale-95'}`}><Icons.Layout size={18} strokeWidth={2} /></button>
                    <button onClick={() => handleTabClick('design')} className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${activeTab === 'design' && isPanelOpen ? 'bg-slate-900 text-white shadow-md shadow-slate-900/20' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-100 hover:scale-105 active:scale-95'}`} title="Шаблоны дизайна"><Icons.Palette size={18} strokeWidth={2} /></button>
                    <button onClick={() => handleTabClick('backgrounds')} className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${activeTab === 'backgrounds' && isPanelOpen ? 'bg-slate-900 text-white shadow-md shadow-slate-900/20' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-100 hover:scale-105 active:scale-95'}`}><Icons.PaintBucket size={18} strokeWidth={2} /></button>
                    <button onClick={() => handleTabClick('text')} className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${activeTab === 'text' && isPanelOpen ? 'bg-slate-900 text-white shadow-md shadow-slate-900/20' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-100 hover:scale-105 active:scale-95'}`}><Icons.Type size={18} strokeWidth={2} /></button>
