...
export const AdminPanel: React.FC<AdminPanelProps> = ({ layouts, onSaveLayout, onDeleteLayout, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-[#111] z-50 flex flex-col text-gray-100 font-sans overflow-hidden">
                                ...
                                <div className="space-y-4">
                                    {orders.map(order => (
                                        <div key={order.id} className="bg-[#1a1a1a] border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition-colors shadow-lg">
                                            ...
                                            <div className="p-5 border-b border-white/10 bg-white/[0.02] flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                                                <div>
                                                    <div className="flex items-center gap-3 mb-1">
                                                        <span className="text-sm font-bold text-white uppercase tracking-wider">Заказ #{order.id.substring(0, 8)}</span>
                                                        <span className="px-2 py-1 bg-amber-500/20 text-amber-400 text-[10px] font-bold uppercase tracking-wider rounded border border-amber-500/20">
                                                            {order.status === 'pending' ? 'Ожидает' : order.status}
                                                        </span>
                                                    </div>
                                                    <div className="flex items-center gap-4 text-xs text-gray-400">
                                                        <span className="flex items-center gap-1.5"><Clock size={12} /> {order.createdAt.toLocaleString('ru-RU')}</span>
                                                        <span className="flex items-center gap-1.5"><UserIcon size={12} /> {order.customerName} ({order.customerEmail})</span>
                                                    </div>
                                                </div>
                                            ...
                                            {/* Order Items */}
                                            <div className="p-5">
                                                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Напечатанные альбомы ({order.items.length})</h4>
                                                    ...
                                                    {order.items.map((item, idx) => (
                                                        <div key={idx} className="flex gap-4 p-4 bg-[#222] rounded-lg border border-white/5 items-center">
                                                            <div className="w-16 h-20 bg-[#111] rounded border border-white/10 overflow-hidden shrink-0 flex items-center justify-center">
                                                                {item.project.previewUrl ? (
                                                                    <img src={item.project.previewUrl} alt="Preview" className="w-full h-full object-cover" />
                                                                ) : (
                                                                    <Icons.Image size={20} className="text-gray-600" />
                                                                )}
                                                            </div>
                                                            <div className="flex-1 min-w-0">
                                                                <h5 className="font-bold text-sm text-gray-200 truncate mb-1">{item.project.name}</h5>
                                                                <p className="text-xs text-gray-500 mb-2">24 страницы • Твердый переплет • {item.quantity} шт.</p>
                                                                <div className="flex items-center justify-between">
                                                                    <span className="text-xs font-bold text-gray-400">{item.pricePerUnit.toLocaleString('ru-RU')} ₽ / шт.</span>
                                                                    <button
                                                                        onClick={() => setExportProject(item.project)}
                                                                        className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 text-xs font-bold rounded transition-colors border border-blue-500/20"
                                                                    >
            ...
            {exportProject && (
                <PDFExporter
                    pages={exportProject.spreads.flatMap(s => [s.leftPage, s.rightPage])}
                    theme={JSON.parse(localStorage.getItem('periodica_theme') || '{}')} // Fallback or pass down actual theme
